...
static void ESP8266_USART_Config ( void )
{
	...
	/* 
		中断配置 
		USART_IT_RXNE寄存器是在每接收到一个字节时产生中断，
		而USART_IT_IDLE寄存器是在接收到完整一帧后产生中断，
		即接收完一个字节在一个字节的时间内再无接收的话就会
		产生中断
	*/
...
bool ESP8266_BuildAP ( char * pSSID, char * pPassWord, ENUM_AP_PsdMode_TypeDef enunPsdMode )
{
	...
	sprintf ( cCmd, "AT+CWSAP_DEF=\"%s\",\"%s\",1,%d", pSSID, pPassWord, enunPsdMode );
...
{
	char cCmd [ 30 ];
	sprintf ( cCmd, "AT+CIPAP_DEF=\"%s\"", pApIp );
	//AT+CIPAP=pApIp  
